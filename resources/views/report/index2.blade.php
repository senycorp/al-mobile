                                ...
                                if (count($data['expenses'])) {
                                    ...
                                    foreach($data['expenses'] as $expense) {
                                        echo    '<tr '.(($expense->hasConflict()) ? 'class="danger"' : null).'>' .
                                                    ...
                                                    '<td style="padding:0">'.(($expense->car_id) ? view('report.fg', ['car' => $expense->car]) : null).'</td>' .
                                                    '<td>'.$expense->getTitle(). (($expense->account) ? '<br/> (Buchung vom Bankkonto)' : null).'</td>' .
                                                    '<td>'.$expense->getIndicatedPrice() . '</td>' .
                                                    '<td>'.($expense->sale_invoice || $expense->purchase_invoice ?  ($expense->sale_invoice) ? view('report.price', ['car' => $expense->car]) : $expense->car->getTaxIdentifier()  : $expense->getTaxIdentifier()).'</td>'.
